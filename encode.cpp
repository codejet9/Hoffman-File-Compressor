...
string convertToString(node* root){
    if(root->left==NULL and root->right==NULL){
        string toReturn; toReturn.push_back('-'); toReturn.push_back(root->val);
        return toReturn;
    }
...
int main(int argv,char* argc[]){
    ...
    while(minHeap.size()>1){
        node* n1=minHeap.top(); minHeap.pop();
        node* n2=minHeap.top(); minHeap.pop();
    ...
    //////// Padding ////////
    ll rem=8-s.length();
    for(ll i=0;i<rem;i++) s.push_back('0');
    ll x=0;
    for(ll j=7;j>=0;j--){
        x=x*2+(s[j]-'0');
    }
